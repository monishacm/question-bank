			...
			<!-- Smart Wizard -->
				<div id="wizard_add_question" class="form_wizard wizard_horizontal">
					...
					<div id="step-1">
						...
						<?php echo $form->field($model, 'chapter_id')->dropDownList($list, array('class' => "select2_group form-control")); ?>
						<?php echo $form->field($model, 'qtype')->dropDownList(['objective' => 'Objective', 'subjective' => 'Subjective'], ['style' => "width: 40%;"]) ?>
						<?php echo $form->field($model, 'description')->textarea(['rows' => 6]) ?>
						<?php echo $form->field($model, 'marks')->textInput(['style' => "width: 40%;"]) ?>
							...
							<?php $i = 0; foreach($model->options as $option) {?>
								<tr>
									...
									<td class="hide-subjective" style="width: 10%;"><?php echo $form->field($option, '['.$i.']correct_answer', [
											'template' => '{input}{error}'
										])->dropDownList([ 'y' => 'Y', 'n' => 'N', ], ['prompt' => '']) ?></td>
								</tr>
							<?php $i ++; }?>
							<?php while($i < 4) {?>
								<tr>
